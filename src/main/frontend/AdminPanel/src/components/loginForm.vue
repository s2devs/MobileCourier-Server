  ...
  <div class="row">
    ...
    <div id="login-form" class="col-sm-4">
      ...
      <div class="form-group">
        <label>Login:</label>
        <input class="form-control" placeholder="Enter your login" v-model="credentials.login"/>
      </div>
      <div class="form-group">
        <label>Password:</label>
          <input class="form-control" placeholder="Enter your password" v-model="credentials.password"/>
      </div>
      <label id="sign-in-btn" @click="signIn">Sign In</label>
      </form>
    </div>
  </div>
</div>
...
#login-form {
  background: #FCFCFC;
  border-radius: 1em;
  box-shadow: 10px 10px 20px 15px #565656;
  letter-spacing: 2px;
  margin-top: 40px;
